...
namespace MinchoCandyWars.Abilities.AmberonCandy
{
    ...
    /// <summary>
    /// 琥珀糖护盾 —— 腰带式个人护盾，核心2级被动。
    /// 破盾后进入 CD，可自动恢复。
    /// </summary>
    public class HediffComp_AmberonShield : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            ticksToNextCheck--;
            if (ticksToNextCheck > 0) return;
            ticksToNextCheck = Props.checkIntervalTicks > 0 ? Props.checkIntervalTicks : 30;

            Pawn shieldTarget = Pawn;
            if (shieldTarget == null || shieldTarget.Dead || !shieldTarget.Spawned) return;
        ...
        {
            if (Pawn == null || !ShouldDisplayGizmo) yield break;
            if (mechShield == null || mechShield.Destroyed) yield break;

            yield return new Gizmo_AmberonShieldHitPoints
            {
        ...
        {
            Pawn target = Pawn;
            if (target == null || target.Dead || target.MapHeld == null || !target.Spawned || ticksToReset > 0) return;
    ...
    public class Gizmo_AmberonShieldHitPoints : Gizmo
    {
        ...
        private static readonly Texture2D FullBarTex = SolidColorMaterials.NewSolidColorTexture(new Color(0.9f, 0.7f, 0.2f));
        private static readonly Texture2D EmptyBarTex = SolidColorMaterials.NewSolidColorTexture(Color.clear);

        public Gizmo_AmberonShieldHitPoints()
        {
            Order = -100f;
        }
